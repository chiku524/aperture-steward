...
- **`OPENAI_*` and embedding env vars apply to the Eliza process (Nosana job env or local `.env`), not Vercel.** Vercel only bridges HTTP via **`AGENT_BASE_URL`**. For Nosana inference use **`OPENAI_API_KEY=nosana`** with Nosana’s OpenAI-compatible **`OPENAI_API_URL`** ending in **`/v1`** (see **`nos_job_def/nosana_eliza_job_definition.json`**). If **`OPENAI_API_URL`** is unset, **`@elizaos/plugin-openai`** falls back to **api.openai.com**, which surfaces misleading “Incorrect API key provided: nosana” errors.
